...
int main(int argc, char **argv)
{
    ...
    for (;;)
    {
        ...
        case 1: // login业务
        {
            ...
            sem_wait(&rwsem); // 等待信号量，子线程处理完登录响应消息后，通知
            if (g_isLoginSuccess)
            {
                // 进入聊天主菜单页面
        ...
        case 2: // register业务
        {
            ...
            sem_wait(&rwsem); // 等待信号量，子线程处理完注册响应消息后，通知
        }
        break;
        case 3: // quit业务
            close(clientfd);
    ...
    else // 登录成功
    {
        ...
        if (responsejs.contains("offlinemsg"))
        {
            ...
            for (string &str : vec)
            {
                ...
                if (ONE_CHAT_MSG == msgjs["msgid"].get<int>())
                {
                ...
                {
                    cout << "群消息[" << msgjs["groupid"].get<int>() << "]:" << msgjs["time"].get<string>() << " [" << msgjs["id"] << "]" << msgjs["name"].get<string>()
                         << " said: " << msgjs["msg"].get<string>() << endl;
                }
            }
        }
...
void doRegResponse(json &responsejs)
{
    ...
    {
        cerr << "name  is already exits, register error!" << endl;
    }
    else // 注册成功
    {
...
void readTaskHandler(int clientfd)
{
    for (;;)
    {
        ...
        {
            doLoginResponse(js); // 处理登录响应业务的逻辑
